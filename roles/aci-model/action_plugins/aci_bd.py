...
class ActionModule(ACIModelPlugin):
	def run(self, tmp=None, task_vars=None, **kwargs):
		...
		new_module_args = self._create_common_module_args(
			"uni/tn-%s/BD-%s"%(module_args['tenant_name'], module_args['bd_name']),
			module_args)

		new_module_args['body'] = self._create_mo('fvBD',
			rn="BD-%s"%module_args['bd_name'],
			name=module_args['bd_name'],
			descr=module_args.get('description',''),
			children=[
				self._create_mo('fvRsCtx',
					rn='rsctx',
					tnFvCtxName=module_args['vrf_name']
				)
			]
			)
